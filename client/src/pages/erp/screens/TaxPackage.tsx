...
export function TaxPackageScreen() {
  ...
  const q = trpc.erp.taxPackage.useQuery({ ym });
  const d = q.data;
    ...
    <>
      ...
      {missing.length > 0 ? (
        <AlertBox>
          ...
          {won(missing.reduce((s, r) => s + (r.amount ?? 0), 0))} — 이 상태로
          넘기면 세무대리인이 다시 물어봅니다. 코드:{" "}
          {missing
            .slice(0, 8)
            .map(r => r.code)
            .join(", ")}
                ...
                <tr key={row.code}>
                  ...
                  <td className="nw">
                    {row.missing ? (
                      ...
                      <span className="chip g" title={row.kinds.join(" · ")}>
                        적격
                      </span>
                    ) : (
                      <span className="chip w" title={row.kinds.join(" · ")}>
                        비적격
                      </span>
                    )}
                  </td>
      ...
      <Note>
        <b>내보내기는 감사 기록에 남습니다.</b> 누가 어느 달 자료를 언제 들고
        나갔는지 통제 화면에서 확인할 수 있습니다. 외부열람 역할은 화면에서 볼 수
        있지만 파일로 내보낼 수 없습니다 — 보는 것과 들고 나가는 것은 다른
        위험입니다.
      </Note>
