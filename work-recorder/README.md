...
터미널에 `bash ` 를 치고 `mac/setup.sh` 를 터미널 창으로 드래그한 뒤 엔터.
`~/Applications/업무녹음.app` 이 만들어지고, 실행하면 상단 상태바에서 녹음 시작·종료와
정리 요청을 할 수 있다. 워커와 스케줄러가 앱 안에서 함께 돌기 때문에 서버를 따로
띄울 필요가 없다. 안내는 [`설치방법.txt`](설치방법.txt), 자세한 내용은
...
| `recorder.py` | 녹음 세션(스트리밍 WAV 기록, 청크 분할)과 업로드 재시도 큐 |
| `clients/mac/` | 메뉴바 앱과 작업 API 클라이언트 |
...
* 앱 번들 코드 서명·공증·자동 업데이트 (Apple 개발자 계정 필요)
* 전역 단축키 — 입력 모니터링 권한 실패 사례가 많아 메뉴바 조작으로 대체
* 시스템 오디오(회의 앱 소리) 캡처 — 가상 오디오 장치와 별도 동의가 필요
* 웹 대시보드, 비용·관측성 대시보드
