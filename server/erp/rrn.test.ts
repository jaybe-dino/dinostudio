...
import { hasRrn, maskRrn, RRN_MASK } from "../../shared/erp/rrn.js";
...
describe("주민번호를 찾는다", () => {
  it("하이픈이 있는 형태", () => {
    const out = maskRrn("주민번호: 930807-2071310");
    expect(out.found).toBe(1);
    expect(out.text).toBe(`주민번호: ${RRN_MASK}`);
  });

  it("「주민」 줄이면 구분자가 없어도 찾는다", () => {
    expect(maskRrn("주민번호:9308072071310").found).toBe(1);
    expect(maskRrn("주민번호:950706 2185314").found).toBe(1);
  });

  it("슬랙이 전화번호로 오인해 감싼 것도 찾는다", () => {
    // 슬랙은 13자리를 <tel:...> 로 감싼다. 그 안의 숫자가 그대로 남으면 안 된다
    const out = maskRrn("주민번호: <tel:9308072071310|930807-2071310>");
...
describe("계좌번호는 건드리지 않는다 — 지워진 계좌로는 송금을 못 한다", () => {
  const accounts = [
    "계좌번호: 1002-251-156248", // 우리은행
    "계좌번호: 3333059947514", // 카카오뱅크 13자리 (구분자 없음)
    "계좌: 81140104329418 국민은행", // 14자리
    "계좌번호 : 기업은행 063-095494-01-014",
    "계좌번호: 356-0006-1039-63", // 농협
    "계좌: 110-387-690168", // 신한
    "계좌번호 12391-04-0356807", // 하나
    "계좌번호: 318002-04-152655", // 국민
    "계좌: 302-0356-7784-11",
  ];

  for (const line of accounts) {
    it(`그대로 둔다 — ${line}`, () => {
      expect(maskRrn(line)).toEqual({ text: line, found: 0 });
    });
  }

  it("사업자등록번호도 그대로 둔다", () => {
    expect(maskRrn("사업자번호 123-45-67890").found).toBe(0);
  });

  it("날짜·금액을 주민번호로 보지 않는다", () => {
    expect(maskRrn("날짜: 250323\n금액: total 150,000원").found).toBe(0);
  });
});
...
describe("검수함 — 화면에는 가려서 나간다", () => {
  ...
  it("목록에는 가려서 나간다 — 프론트가 아니라 여기서 가린다", async () => {
    ...
    // 계좌번호는 살아 있어야 한다 — 이 돈을 실제로 보내야 한다
    expect(intake.raw).toContain("1002-251-156248");
  });
...
describe("hasRrn", () => {
  it("빈 값에 놀라지 않는다", () => {
    expect(hasRrn(null)).toBe(false);
    expect(hasRrn(undefined)).toBe(false);
    expect(hasRrn("")).toBe(false);
  });
});
